...
cur_struct = MRIread([mypath, '/mask/mask_ERC_harddraw_2mm.nii']);
...
export_table = NaN(length(sub_pool_num)*4*12, 4);
counter = 1;
for sub = sub_pool_num
    cur_sub = sub_pool{sub};
    for sess = 2:2:8
        % access raw table
        mypath_time_record =   [mypath, '/sub', num2str(sub), '_mri_record_no_calibration_sinusoid_testing_for_erc_handdraw_6fold.txt'];
        timeFile = fopen(mypath_time_record,'rt');
        table_time = textscan(timeFile,'%s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s');
        fclose(timeFile);
        
        %keyword
        roi_str_cell = extractBetween(mypath_time_record, 'testing_for_', '.txt');
        ...
        cur_struct = MRIread([mypath, '/liujia_formal_sub', cur_sub,'/periodic_2403_testing_6_fieldmap_no_cali_handerc_s', num2str(sess), '.feat/stats/standard_pe1.nii.gz']);
...
export_table2 = NaN(length(sub_pool_num), 12);
angle_pool = 0:30:330;
for sub = sub_pool_num
...
fileID = fopen([mypath, '/fig_MRI_fig_2/txt_barchart_6_fold_effect_erc_handdraw_g12.txt'],'w');
