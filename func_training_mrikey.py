...
def training(screen_width, screen_height, window, entry, application_path, stimuli_width, stimuli_height,
    ...
    def trial_maker(is_formal, with_feedback, is_ending):
        ...
        if with_feedback:
            ...
            trial_type = "testing"
        if is_formal:
            num_trial = len(shuffled_session_image_path_pool) # 实验前取消注释
            # num_trial = 2 # 实验前注释掉这一行
        else:
            num_trial = 4 # change to 4
        for ith_trial in list(range(0, num_trial)):
            #
            show_fixation()  # show fix
            # give_a_fixed_time_interval(duration=1500)  # give interval
        ...
        return next_while, end_screen_while
    ...
    while in_game:
        ...
        while practice_while:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN and event.key == pygame.K_RETURN:
                    # pygame.event.clear()
                    practice_while = False
                    formal_inst_while, end_screen_while = trial_maker(is_formal=False, with_feedback=True, is_ending=False)
                ...
                while in_session:
                    ...
                    for event in pygame.event.get():
                        ...
                        if event.type == pygame.KEYDOWN and event.key == pygame.K_RETURN:
                            # pygame.event.clear()
                            in_session = False
                            next_while, end_screen_while = trial_maker(is_formal=True, with_feedback=True, is_ending=False)
        ...
        while test_session_while:
            next_while, end_screen_while = trial_maker(is_formal=True, with_feedback=False, is_ending=True)
            test_session_while = False
            end_screen_while = True
