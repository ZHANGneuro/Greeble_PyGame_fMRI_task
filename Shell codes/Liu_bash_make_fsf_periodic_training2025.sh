...
for sub in {21..35}; do
  ...
  for session in 1 3 5 7; do
    ...
    OUTPUTDIR=$server_root"/liujia_formal_sub"$sub"/periodic_2501_"$fiting_type"_"$Parameter"_fieldmap_"$calibration_status"_s"$session
    ...
    -e 's@Standard_brain_dir@'$Standard_brain_dir'@g' <$TEMPLATE_PATH> "/Users/bo/Documents/script_bash/template_periodic_training_fieldmap2025/fsf_"$Parameter"_sub"$sub"_s"$session"_"$calibration_status".fsf"
  done
done
