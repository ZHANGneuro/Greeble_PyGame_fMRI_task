...
for sub in {1..20}; do
  ...
  for session in 2 4 6 8; do
    ...
    Parameter="3"
    calibration_status="no_cali" #  cali  no_cali
    fiting_type="training" #training testing
    ...
    OUTPUTDIR=$server_root"/MRI_result_sinusoid/liujia_formal_sub"$ith_sub_str"/periodic_2403_"$fiting_type"_"$Parameter"_fieldmap_"$calibration_status"_s"$session
    BOLD_IMAGE=$server_root"/MRI_filtered_bold_nosmooth_fieldmap/filtered_nosmooth_sub"$sub"_s"$session".nii.gz"
    EV_file_name_sin="sub"$sub"_s"$session"_obj_sin"$Parameter"_"$calibration_status".txt"
    EV_file_name_cos="sub"$sub"_s"$session"_obj_cos"$Parameter"_"$calibration_status".txt"
    ...
    -e 's@EV_file_name_sin@'$EV_file_name_sin'@g' \
    -e 's@EV_file_name_cos@'$EV_file_name_cos'@g' \
    ...
    -e 's@Standard_brain_dir@'$Standard_brain_dir'@g' <$TEMPLATE_PATH> "/Users/bo/Documents/script_bash/template_periodic_training_para3_2024/fsf_"$Parameter"_sub"$sub"_s"$session"_"$calibration_status".fsf"
  done
done
