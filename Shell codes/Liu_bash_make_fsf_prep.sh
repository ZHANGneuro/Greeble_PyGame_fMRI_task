...
TEMPLATE_PATH=/Users/bo/Documents/script_bash/batch_prepro_fieldmap_template.fsf
for sub in {21..35}; do
  for session in {1..8}; do
    ...
    OUTPUTDIR=$server_root"/MRI_preprocess/preprocessing_fieldmap_sub"$sub"_s"$session
    T1_IMAGE=$server_root"/MRI_raw_t1/sub"$sub"_t1_brain"
    BOLD_IMAGE=$server_root"/MRI_raw_bold_4d/sub"$sub"_s"$session
    ...
    field_map_phase=$server_root"/MRI_fieldmap_pha/sub"$sub"_fieldmap_pha_rad"
    field_map_mag=$server_root"/MRI_fieldmap_mag/sub"$sub"_fieldmap_mag_brain"
    ...
    -e 's@BOLD_IMAGE@'$BOLD_IMAGE'@g' <$TEMPLATE_PATH> /Users/bo/Documents/script_bash/template_liujia_prep_mri_fieldmap2025/fsf_sub$sub"_s"$session".fsf"
